...
add_filter( 'gform_pre_render_1', 'typewheel_limit_countries' );
add_filter( 'gform_pre_validation_1', 'typewheel_limit_countries' );
add_filter( 'gform_pre_submission_filter_1', 'typewheel_limit_countries' );
add_filter( 'gform_admin_pre_render_1', 'typewheel_limit_countries' );
...
require_once get_stylesheet_directory() . '/functions/campaign-functions.php';

// Add tracking codes to head
add_action('wp_head', 'add_tracking_codes_to_head');
function add_tracking_codes_to_head() {
    $title = wp_get_document_title();

    // Output tracking code to pages/posts that have "thank you" in the title
    if (stripos($title, 'thank you') !== false) {
		echo '<script type="text/javascript" src="https://click.appcast.io/pixels/generic3-6070.js?ent=147"></script>';
    } else {
		// Output tracking code not meant for thank you pages
		echo '<script type="text/javascript" src="https://click.appcast.io/pixels/generic1-6070.js?ent=147"></script>';
	}
}

// Add tracking codes to body
add_action('wp_footer', 'add_tracking_codes_to_body');
function add_tracking_codes_to_body() {
    $title = wp_get_document_title();

    // Output tracking code to pages/posts that have "thank you" in the title
    if (stripos($title, 'thank you') !== false) {
		echo '<img src="https://www.jobs2careers.com/conversion2.php?p=2149" width="1" height="1" />';
    } else {
		// Output any tracking code not meant for thank you pages
		return;
	}
}

// Counts the amount of leads that have a campaign name
function count_leads_for_campaign($campaign_id) {
    // Get the campaign's own utm_campaign field value
    $utm_campaign = get_post_meta($campaign_id, 'utm_campaign', true);
	...
	// Create link to leads CPT
	$url = add_query_arg([
		'post_type' => 'lead',
		'acp_filter[e4268ab6b020c8]' => rawurlencode($utm_campaign),
	], admin_url('edit.php'));
...
// Filter to target the "leads" column in the campaign cpt and populate it.
add_filter('ac/column/value', function ($value, $post_id, $column) {
    if ('campaign' !== get_post_type($post_id)) {
        return $value;
    }
    if ('440415ca87ef9c' === $column->get_name()) {
        $new_value = count_leads_for_campaign($post_id);
        return $new_value;
    }
    return $value;
}, 10, 3);
